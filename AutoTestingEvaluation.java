...
public class AutoTestingEvaluation {
    ...
    public static Double[] F_Instrumented_Tracey(Population pop) {
        double phenotype;
        ...
        for (String[] aPopAL : popAL) {
            ...
            System.out.println("[F_Instrumented_Tracey()] Individual #" + k + ": " + aPopAL[k] + " and fitness{" + k + "] = " + fitness[k]);
            ++k;
        }
        
//        for (int count = 0; count < x.length; count++) {
//            System.out.println("F_Instrumented_Tracey() [Solution]: x" + count + " = " + x[count]);
//        }
        
        return fitness;
    }
    
	public static double F_Instrumented_Tracey(double[] x) {
		double xx = x[0];
		double yy = x[1];
		double fitness = 0d;
...
//		System.out.printf("*4 [z = %.4f] ==> ", z);			
		if ((z > 1d) && (z <= 5d)) {
			...
			executed += 1d;	
//			System.out.print("*5 ==> ");			
			if ( (z >= 2d - Z_BEARANCE/2d) && (z <= 2d + Z_BEARANCE/2d) ) {	// Division-by-Zero Node.
				branch_distance += 0d;
				executed += 1d;
